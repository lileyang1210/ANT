...
from src.dataset import ANTDataset
import gc
import sys
...
def main(args):   
    ...
    for single_concept in train_dataset.dict_for_close_form:
        ...
        lora_path = f"{args.lora_path}/{single_concept['old'][0][1]}/{single_concept['old'][0][1]}_last.safetensors"
        lora_pipe.load_lora_weights(lora_path)
        lora_pipe.fuse_lora(lora_scale=lora_scale)
    ...
    final_path = "saved_model/" + lora_path.replace(f"/{single_concept['old'][0][1]}/{single_concept['old'][0][1]}_last.safetensors",
                                                     f"_{lora_scale}_Pscale{args.train_preserve_scale}_Pweight{args.preserve_weight}")
    final_pipe.save_pretrained(final_path)
